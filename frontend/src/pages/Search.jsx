...
const Search = () => {
  ...
  const applyFilter = () => {
    ...
    if (menu === "movie" || menu === "series") {
      filtered = filtered.filter((item) => item.category.toLowerCase() === menu);
    } else if (menu !== "All") {
      let menuLower = menu.toLowerCase();
      filtered = filtered.filter((item) => {
        let itemGenres =
          typeof item.genre === Array
            ? item.genre.split(",").map((g) => g.trim().toLowerCase())
            : Array.isArray(item.genre)
            ? item.genre.filter((g) => typeof g === "string").map((g) => g.toLowerCase())
            : [];
        return itemGenres.includes(menuLower);
      });
    }
      ...
      <div className="w-full h-full lg:flex py-5 px-5 lg:py-10 lg:px-5">
        <div className="lg:w-1/4 w-full">
          ...
          <div className="w-full flex items-center lg:items-start lg:flex-col gap-3 py-2 lg:py-32 lg:px-3 lg:text-[20px] overflow-scroll show-scroll">
            {["All", "movie", "series"].map((category) => (
              <button
                key={category}
                onClick={() => setMenu(category)}
                className={`py-2 w-full rounded-md px-2 text-xl cursor-pointer text-start transition-all ${
                  menu === category ? "backdrop-blur-3xl bg-white/20 text-white font-semibold" : "text-gray-300"
                }`}
              >
          ...
          <div className="lg:grid lg:grid-cols-4 gap-3 py-4 h-full flex-col-reverse">
            {loading ? (
              ...
              filteredData.map((item, index) => (
                <Link
                  to={
                    item.category.toLowerCase() === "series"
                ...
                >
                  <img src={item.image[0]} alt="image" className="lg:block hidden w-full" />
                  <div className="flex gap-3 lg:hidden mt-5">
                    ...
                    <div className="flex flex-col justify-between mt-2">
                      <div className="flex flex-col gap-2">
                        <div>
                          <h1 className="text-xl line-clamp-1">{item.series_name || item?.title}</h1>
                          <p className="text-gray-500">{item?.category}</p>
                        </div>
                        <div className="flex gap-2 items-center w-full">
                          <span className="bg-white text-black px-1 rounded-md text-xs">IMDB</span>
